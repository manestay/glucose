${T5_ENV}/bin/t5_mesh_transformer \
    ...
    --model_dir="${MODEL_DIR}/finetuned" --gin_file="${MODEL_DIR}/large/operative_config.gin" \
    --gin_file="${GIN_DIR}/sample_decode.gin" --gin_file="${GIN_DIR}/infer.gin" \
    --gin_param="utils.run.mode = 'infer'" --gin_param="infer_checkpoint_step = 13157000" \
